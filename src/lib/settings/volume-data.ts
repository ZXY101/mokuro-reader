...
/**
 * Merges local and remote volume data, keeping the newest progress while preserving metadata
 * @param localVolumes Local volume data
 * @param remoteVolumes Remote volume data (from any sync provider)
...
export function mergeVolumeData(
  ...
  const allVolumeIds = new Set([
    ...
    ...Object.keys(remoteVolumes)
  ]);

  allVolumeIds.forEach(volumeId => {
    const local = localVolumes[volumeId];
    const remote = remoteVolumes[volumeId];

    if (!local) {
      merged[volumeId] = remote;
    } else if (!remote) {
      ...
      const remoteDate = new Date(remote.lastProgressUpdate).getTime();
      const newerRecord = localDate >= remoteDate ? local : remote;
      const olderRecord = localDate >= remoteDate ? remote : local;

      // Fill in missing metadata from the other record
      merged[volumeId] = new VolumeData({
        ...newerRecord,
...
/**
 * Enrich all volumes with missing metadata from IndexedDB
 * This should be called on app startup to ensure stats work even after volumes are deleted
 */
export async function enrichAllVolumesWithMetadata(): Promise<void> {
  if (!browser) return;

  try {
    const currentVolumes = Object.entries(await new Promise<Volumes>(resolve => {
      volumes.subscribe(value => resolve(value))();
    }));

    // Find volumes missing metadata
    const volumesToEnrich = currentVolumes.filter(([_, data]) =>
      !data.series_uuid || !data.series_title || !data.volume_title
    );

    if (volumesToEnrich.length === 0) {
      return;
    }

    console.log(`[Volume Data] Enriching ${volumesToEnrich.length} volumes with metadata from IndexedDB...`);

    // Enrich each volume
    let enrichedCount = 0;
    for (const [volumeId, data] of volumesToEnrich) {
      try {
        const metadata = await db.volumes.get(volumeId);
        if (metadata) {
          volumes.update(vols => ({
            ...vols,
            [volumeId]: new VolumeData({
              ...data,
              series_uuid: metadata.series_uuid,
              series_title: metadata.series_title,
              volume_title: metadata.volume_title
        ...
        console.warn(`Failed to enrich metadata for volume ${volumeId}:`, error);
      }
    }

    console.log(`[Volume Data] Enriched ${enrichedCount}/${volumesToEnrich.length} volumes`);
  } catch (error) {
    console.error('[Volume Data] Failed to enrich volumes:', error);
  }
}
