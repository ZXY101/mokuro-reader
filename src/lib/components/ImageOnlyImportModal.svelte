...
<Modal bind:open size="md" outsideclose on:close={handleCancel}>
  <div class="flex flex-col gap-4">
    ...
    <p class="text-sm text-gray-600 dark:text-gray-400 text-center">
      Found {$imageOnlyImportModalStore?.totalVolumes ?? 0} volume(s) in {$imageOnlyImportModalStore?.seriesList?.length ?? 0} series without .mokuro files.
      These will be imported as image-only volumes (no OCR text).
    </p>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0">
          <tr>
            ...
            <th class="px-4 py-2 text-right font-medium text-gray-700 dark:text-gray-300">Volumes</th>
          </tr>
        </thead>
        <tbody class="divide-y dark:divide-gray-600">
          {#each $imageOnlyImportModalStore?.seriesList ?? [] as series}
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
              ...
              <td class="px-4 py-2 text-right text-gray-600 dark:text-gray-400">{series.volumeCount}</td>
            </tr>
          {/each}
        </tbody>
      </table>
