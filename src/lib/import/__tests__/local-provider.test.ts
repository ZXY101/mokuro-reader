...
describe('localImportProvider', () => {
	...
	describe('unsupported operations', () => {
		...
		it('throws on downloadFile', async () => {
			await expect(
				localImportProvider.downloadFile({ provider: 'google-drive', fileId: '123', path: '', modifiedTime: '', size: 0 })
			).rejects.toThrow(/does not support/i);
		});

		it('throws on deleteFile', async () => {
			await expect(
				localImportProvider.deleteFile({ provider: 'google-drive', fileId: '123', path: '', modifiedTime: '', size: 0 })
			).rejects.toThrow(/does not support/i);
		});
