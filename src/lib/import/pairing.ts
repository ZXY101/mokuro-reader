...
export async function pairMokuroWithSources(entries: FileEntry[]): Promise<PairingResult> {
  ...
  const unpairedDirs = findUnpairedImageDirectories(imageFiles, pairedImageDirs, dirStructure);
 ...
 */
function buildDirectoryStructure(imageFiles: CategorizedFile[]): Map<string, Map<string, File>> {
 ...
 */
function createArchivePairing(archive: CategorizedFile, mokuroFile: File | null): PairedSource {
